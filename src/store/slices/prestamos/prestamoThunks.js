...
import { addPrestamo, deletePrestamo, loadPrestamos, modifyPrestamo } from "./prestamoSlice";

export const newPrestamo = (prestamo) => {
  return async (dispatch) => {
    try {
      const prestamoResponse = await fetchWithToken(
        "prestamo",
        prestamo,
        "POST"
      );
      ...
      dispatch(addPrestamo(body.prestamo));
    } catch (error) {
      console.log(error);
...
export const startModifyPrestamo = (prestamo) => {
  return async (dispatch) => {
    try {
      const responsePin = await fetchWithToken(
        "auth/pin",
        { pin: prestamo.pin },
        "POST"
      );
      ...
      const responsePrestamo = await fetchWithToken(
...
export const startDeletePrestamo = (prestamo) => {
  return async (dispatch) => {
    try {
      const responsePin = await fetchWithToken(
        "auth/pin",
      ...
      Swal.fire("Éxito!", "Se eliminó el prestamo", "success");

      dispatch(deletePrestamo(prestamo));
    } catch (error) {
      Swal.fire("Ups!", "Se presentó un error", "warning");
    }
  };
